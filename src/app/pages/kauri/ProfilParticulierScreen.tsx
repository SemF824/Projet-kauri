import {
  ...
  Home, Wallet, Briefcase, Rocket, ArrowRight, FolderOpen,
} from 'lucide-react';
...
export function ProfilParticulierScreen() {
  ...
  // Correction chirurgicale : trustScore est directement extrait sur son échelle 0-100 réelle
  ...
  function SettingRow({ icon: Icon, color, bg: ibg, label, sub, onClick }: { icon: any; color: string; bg: string; label: string; sub: string; onClick?: () => void }) {
    return (
      <button onClick={onClick} className="w-full rounded-2xl p-4 flex items-center justify-between cursor-pointer" style={{ backgroundColor: card, border: `1.5px solid ${border}` }}>
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xl font-bold" style={{ color: GOLD }}>{trustScore} / 100</span>
          </div>
          <div className="h-2 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.15)' }}>
            <div className="h-full rounded-full transition-all duration-500" style={{ width: `${trustScore}%`, background: `linear-gradient(90deg, ${GOLD}, #F59E0B)` }} />
          </div>
        ...
        >
          ...
          >
            <User style={{ width: 15, height: 15, color: activeSpace === 'personnel' ? TEAL : 'rgba(255,255,255,0.55)', strokeWidth: 2 }} />
            <span className="text-sm font-semibold" style={{ color: activeSpace === 'personnel' ? TEAL : 'rgba(255,255,255,0.55)' }}>
              Espace Personnel
            </span>
          </button>
          ...
          >
            <Briefcase style={{ width: 15, height: 15, color: activeSpace === 'professionnel' ? TEAL : 'rgba(255,255,255,0.55)', strokeWidth: 2 }} />
            <span className="text-sm font-semibold" style={{ color: activeSpace === 'professionnel' ? TEAL : 'rgba(255,255,255,0.55)' }}>
              Pro
            </span>
          </button>
        </div>
      ...
      <div className="px-5 pt-5 space-y-5">
          ...
          <>
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {[
                  { icon: Mail,   color: TEAL,      ibg: `${TEAL}12`,    label: 'Email',     sub: user?.email || 'Non renseigné' },
                  ...
                  <button key={label} onClick={() => navigate('/kauri/manage-account')} className="w-full rounded-2xl p-4 flex items-center justify-between cursor-pointer" style={{ backgroundColor: card, border: `1.5px solid ${border}` }}>
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: ibg }}>
            ...
            <div>
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  <button key={card_.number} onClick={() => navigate('/kauri/portefeuille')} className="w-full rounded-2xl p-4 flex items-center justify-between cursor-pointer" style={{ backgroundColor: card, border: `1.5px solid ${border}` }}>
                    <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="space-y-2.5">
                ...
                <button onClick={() => navigate('/kauri/coffre-numerique')} className="w-full rounded-2xl p-4 flex items-center justify-between cursor-pointer" style={{ backgroundColor: card, border: `1.5px solid ${border}` }}>
                  <div className="flex items-center gap-3">
            ...
            <div onClick={() => navigate('/kauri/premium-paywall')} className="rounded-3xl p-6 relative overflow-hidden cursor-pointer" style={{ background: 'linear-gradient(135deg, #D4AF37 0%, #B8860B 60%, #D4AF37 100%)', boxShadow: '0 8px 28px rgba(212,175,55,0.40)' }}>
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center justify-between rounded-2xl p-3" style={{ backgroundColor: 'rgba(255,255,255,0.18)' }}>
                  ...
                  <button className="px-4 py-2 rounded-xl text-sm font-bold cursor-pointer" style={{ backgroundColor: '#fff', color: '#B8860B' }}>
                    Découvrir
                  </button>
                </div>
              </div>
            ...
            <div className="rounded-3xl p-5 relative overflow-hidden" style={{ background: isDarkMode ? 'linear-gradient(135deg, #1A1200 0%, #2A1E00 100%)' : 'linear-gradient(135deg, #FFFBEB 0%, #FEF3C7 100%)', border: `2px solid ${GOLD}55`, boxShadow: `0 6px 28px ${GOLD}22` }}>
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: `linear-gradient(135deg, ${GOLD}, #B8860B)`, boxShadow: `0 4px 14px ${GOLD}55` }}>
              ...
              <p className="text-sm leading-relaxed mb-5" style={{ color: isDarkMode ? 'rgba(255,255,255,0.70)' : '#78450A' }}>
                Propulsez votre propre projet. Débloquez les outils de financement participatif pour <span style={{ fontWeight: 700, color: isDarkMode ? GOLD : '#92400E' }}>collecter des dons</span> ou des <span style={{ fontWeight: 700, color: isDarkMode ? GOLD : '#92400E' }}>investissements</span>.
              </p>
        ...
        {activeSpace === 'professionnel' && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-20 h-20 rounded-3xl flex items-center justify-center mb-5 mx-auto" style={{ background: `linear-gradient(135deg, ${GOLD}, #B8860B)`, boxShadow: `0 6px 20px ${GOLD}44` }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto" style={{ backgroundColor: isDarkMode ? '#1E293B' : '#fff', borderTop: `1px solid ${isDarkMode ? '#334155' : '#E8EDF2'}`, paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
        <div className="flex items-end justify-around px-2 pt-1.5 pb-3">
          {[
            ...
            const Icon = tab.icon;
            return (
              <button key={tab.id} onClick={() => navigate(tab.path)} className="flex flex-col items-center gap-0.5 cursor-pointer" style={{ minWidth: 44, paddingTop: 6, paddingBottom: 2 }}>
          ...
          <button onClick={() => navigate('/kauri/tontines-actives')} style={{ width: 56, height: 56, borderRadius: '50%', display: 'flex', items: 'center', justifycontent: 'center', boxShadow: '0 4px 16px rgba(212,175,55,0.5), 0 2px 8px rgba(0,0,0,0.18)', border: '2px solid rgba(255,255,255,0.2)', marginBottom: 6, flexShrink: 0, background: 'linear-gradient(135deg, #D4AF37, #F59E0B)', cursor: 'pointer' }}>
          ...
          <button onClick={() => navigate('/kauri/social-hub-gateway')} className="flex flex-col items-center gap-0.5 cursor-pointer" style={{ minWidth: 44, paddingTop: 6, paddingBottom: 2 }}>
