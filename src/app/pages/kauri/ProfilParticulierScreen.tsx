...
export function ProfilParticulierScreen() {
  ...
  // États de gestion de la clé biométrique
  const [isPasskeyActive, setIsPasskeyActive] = useState(false);
  const [isActivatingBio, setIsActivatingBio] = useState(false);

  const { profile, user, signOut } = useAuth();

  // Alignement sur l'échelle 40/100 nominale définie en base de données pour le nouvel utilisateur
  const trustScore =
  ...
  // Synchronisation de l'état local de la clé d'accès
  useEffect(() => {
    if (user?.email) {
      const savedBio = localStorage.getItem(`kauri_bio_active_${user.email}`);
      setIsPasskeyActive(savedBio === "true");
    }
  }, [user]);

  // Déclencheur et enregistrement de la clé d'accès (Face ID / Passkey)
  const handleTogglePasskey = async () => {
    if (!user?.email) return;

    if (isPasskeyActive) {
      localStorage.removeItem(`kauri_bio_active_${user.email}`);
      localStorage.removeItem("kauri_rememberED_email");
      setIsPasskeyActive(false);
      toast.success("Clé d’accès biométrique révoquée avec succès.");
      return;
    }

    setIsActivatingBio(true);
    toast.info("Génération du défi sécurisé matériel (WebAuthn)...");

    try {
      // Simulation du délai d'interrogation de l'enclave sécurisée de l'appareil
      await new Promise((resolve) => setTimeout(resolve, 1800));
      
      localStorage.setItem(`kauri_bio_active_${user.email}`, "true");
      localStorage.setItem("kauri_rememberED_email", user.email); // Requis pour l'accès direct depuis KauriLoginScreen
      setIsPasskeyActive(true);
      toast.success("Clé d’accès enregistrée ! Face ID / Touch ID activés.");
    } catch (err) {
      toast.error("Échec de la liaison biométrique matérielle.");
    } finally {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: textS }}>
              {sub}
            </p>
          </div>
        </div>
        <ChevronRight
          style={{ width: 16, height: 16, color: inactive }}
        />
      </button>
    );
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="text-white/60 text-xs mt-1.5">
            {trustScore >= 85 ? "Excellent · Membre de confiance" : "Niveau initial · À compléter par vos actions"}
          </p>
        </div>
      ...
      <div className="px-5 pt-5 space-y-5">
        {activeSpace === "personnel" && (
          <>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                },
                {
                  ...
                  label: "Investis",
                  value: "2",
                  path: "/kauri/mes-investissements",
                },
                {
                  icon: Award,
                  color: "#8B5CF6",
                ...
                ({
                  ...
                  >
                    ...
                    >
                      <Icon
                        style={{ width: 19, height: 19, color }}
                      />
                    </div>
                    <p
              ...
              <div className="space-y-2.5">
                {[
                  ...
                  },
                  {
                    ...
                    ibg: "#3B82F612",
                    label: "Téléphone",
                    sub: profile?.phone || "Non renseigné",
                  },
                  {
                    icon: MapPin,
                    color: "#8B5CF6",
                  ...
                  ({ icon: Icon, color, ibg, label, sub }) => (
                    <button
                      key={label}
                      onClick={() =>
                        navigate("/kauri/manage-account")
                      }
                      className="w-full rounded-2xl p-4 flex items-center justify-between cursor-pointer text-left outline-none"
                      style={{
                        backgroundColor: card,
            ...
            <div>
              ...
              <div className="space-y-2.5">
                <SettingRow
                  ...
                  onClick={() =>
                    navigate("/kauri/biometric-setup")
                  }
                />
                
                {/* Ligne Clé Biométrique active et interactive */}
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      ...
                      style={{ backgroundColor: isPasskeyActive ? `${GOLD}18` : (isDarkMode ? '#334155' : '#E2E8F0') }}
                    >
                      {isActivatingBio ? (
                        <div className="w-4 h-4 rounded-full border-2 border-amber-500 border-t-transparent animate-spin" />
                      ) : (
                        <Smartphone style={{ width: 19, height: 19, color: isPasskeyActive ? GOLD : (isDarkMode ? '#94A3B8' : '#64748B') }} />
                      )}
                    </div>
                    <div>
                      <p className="text-sm font-medium" style={{ color: textP }}>
                        Clé d'accès biométrique
                      </p>
                      <p className="text-xs" style={{ color: isPasskeyActive ? '#10B981' : textS, fontWeight: isPasskeyActive ? 600 : 400 }}>
                        {isPasskeyActive ? "Face ID / Touch ID activé" : "Associer l'empreinte de l'appareil"}
                      </p>
                    </div>
                  </div>
                ...
                <SettingRow
                  ...
                  sub="Désactivé"
                  onClick={() => navigate("/kauri/setup-2fa")}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2.5">
                ...
                <SettingRow
                  ...
                  sub="Contrôler vos données"
                  onClick={() =>
                    navigate("/kauri/preferences-contenu")
                  }
                />

                <button
                  onClick={() =>
                    navigate("/kauri/coffre-numerique")
                  }
                  className="w-full rounded-2xl p-4 flex items-center justify-between cursor-pointer text-left outline-none"
                  style={{
                    backgroundColor: card,
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p
                        ...
                        style={{ color: textS }}
                      >
                        Contrats signés · Coffre-fort numérique
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, #D4AF37 0%, #B8860B 60%, #D4AF37 100%)",
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-1.5 mb-4">
                  {[
                    {
                      icon: Zap,
                      text: "SWAPs illimités gratuits",
                    },
                    {
                      icon: CheckCircle2,
                      text: "Carte physique offerte",
                    },
                    {
                      icon: TrendingUp,
                      text: "Frais réduits de 50%",
                    },
                  ].map(({ icon: Icon, text }) => (
                    <div
                      key={text}
            ...
            >
              ...
              >
                Propulsez votre propre projet. Débloquez les
                outils de financement participatif pour{" "}
                <span
                  style={{
                    fontWeight: 700,
                    color: isDarkMode ? GOLD : "#92400E",
                  }}
                >
                  collecter des dons
                </span>{" "}
              ...
              <button
                onClick={() =>
                  navigate("/kauri/pro-verification")
                }
                className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-2xl text-sm font-bold transition-all active:scale-[0.98] cursor-pointer"
                style={{
                  background: `linear-gradient(135deg, ${GOLD}, #B8860B)`,
                  color: "#fff",
            ...
            <button
              ...
              className="w-full rounded-2xl p-4 flex items-center justify-center gap-2 text-sm font-semibold cursor-pointer"
              style={{
                background:
                  "linear-gradient(135deg, #B05B3B, #DC2626)",
                color: "#fff",
        ...
        {activeSpace === "professionnel" && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            >
              Activez votre profil Pro pour accéder aux outils
              de financement participatif et piloter vos
              projets.
            </p>
            <button
              onClick={() =>
              ...
              className="flex items-center gap-2.5 px-6 py-3.5 rounded-2xl text-sm font-bold cursor-pointer"
              style={{
                background: `linear-gradient(135deg, ${GOLD}, #B8860B)`,
                color: "#fff",
      ...
      <nav
        ...
        style={{
          ...
          borderTop: `1px solid ${isDarkMode ? "#334155" : "#E8EDF2"}`,
        ...
        <div className="flex items-end justify-around px-2 pt-1.5 pb-3">
          {[
            {
              ...
              icon: Home,
              label: "Accueil",
              path: "/kauri/normal-dashboard",
            },
            {
              id: "investissement",
              icon: Wallet,
              label: "Investissement",
              path: "/kauri/investissement",
            },
          ].map((tab) => {
            const Icon = tab.icon;
            return (
              <button
                ...
                className="flex flex-col items-center gap-0.5 cursor-pointer text-center outline-none"
                style={{
                  minWidth: 44,
                  paddingTop: 6,
                  paddingBottom: 2,
              ...
              >
                ...
                />
                <span style={{ fontSize: 10, color: inactive }}>
                  {tab.label}
                </span>
              </button>
            );
          ...
          <button
            ...
            style={{
              ...
              boxShadow:
                "0 4px 16px rgba(212,175,55,0.5), 0 2px 8px rgba(0,0,0,0.18)",
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M3 12 Q7 9.5 12 9.5 Q17 9.5 21 12"
                stroke={inactive}
                strokeWidth="1.2"
                strokeLinecap="round"
                fill="none"
              ...
              />
              <path
                d="M15.8 13.5 Q17 12.5 18.2 13.5"
                stroke={inactive}
                strokeWidth="1.1"
                strokeLinecap="round"
                fill="none"
